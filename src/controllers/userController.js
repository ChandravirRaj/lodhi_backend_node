...
const registerUser = async (req, res) => {
  try {
    ...
    // Create new user
    const newUser = new User({ firstName, lastName, email, phoneNumber, countryCode, password: hashedPassword });
...
const loginUser = async (req, res) => {
 ...
 const getAllUsers = async (req,res) =>{
    try{
      ...
      sendResponse(res,200,'Users data fetched successfully',allUsers);
    }catch(err){
      sendResponse(res,500,err.message)
    }
  };
